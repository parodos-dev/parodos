...
public class NotificationMessageCreateRequestDTO {
  ...
  * @throws IOException if the JSON Object is invalid with respect to NotificationMessageCreateRequestDTO
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!NotificationMessageCreateRequestDTO.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in NotificationMessageCreateRequestDTO is not found in the empty JSON string", NotificationMessageCreateRequestDTO.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!NotificationMessageCreateRequestDTO.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `NotificationMessageCreateRequestDTO` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!NotificationMessageCreateRequestDTO.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'NotificationMessageCreateRequestDTO' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<NotificationMessageCreateRequestDTO> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(NotificationMessageCreateRequestDTO.class));
  ...
  * @throws IOException if the JSON string is invalid with respect to NotificationMessageCreateRequestDTO
  */
