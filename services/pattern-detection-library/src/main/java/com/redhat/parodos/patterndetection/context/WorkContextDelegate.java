...
public class WorkContextDelegate {
	 ...
	 * - START_DIRECTORY : the root location to begin processing the files
	 * - START_FILE : the root java.util.File to begin processing the files
	 * 
	...
	public boolean validateAndIntializeContext(WorkContext context) {
		...
		if (context.get(WorkFlowConstants.START_DIRECTORY.toString()) == null && context.get(WorkFlowConstants.START_FILE.toString()) == null) {
			log.error("No target destination to scan defined. Ensure the WorkContext contains an {} entry of type String, or an {} entry of type String",WorkFlowConstants.START_DIRECTORY.toString(), WorkFlowConstants.START_FILE.toString());
			return false;
		}
	...
	public boolean areAllPatternsDetected(WorkReport report) {
		...
		int numberOfTargetStatesRequired = ((List<Pattern>) report.getWorkContext().get(WorkFlowConstants.DESIRED_PATTERNS.toString()))
				.size();
		@SuppressWarnings("unchecked")
		int numberOfTargetStatesFound = ((List<Pattern>) report.getWorkContext().get(WorkFlowConstants.DETECTED_PATTERNS.toString()))
				.size();
	...
	public boolean isThisPatternDetected(Pattern pattern, WorkReport report) {
		for (Pattern thisPattern : (List<Pattern>) report.getWorkContext().get(WorkFlowConstants.DETECTED_PATTERNS.toString())) {
	...
	/**
	 * This checks all the DESIRED_PATTERNS and compares them to the detected CLUE(S) to see
	 * if any of the DESIRED_PATTERNS could become DETECTED_PATTERNS
	 * 
	 * @param context WorkContext used during the Scan 
	 */
	public void processResultsAfterScan(WorkContext context) {
		for (Pattern pattern : getDesiredPatterns(context)) {
			...
			if (needAllConditionsRequired && needOneOfConditionsRequired) {
				...
				patternDetected = (needAllConditionsRequired && needAllConditionsAchieved) || (needOneOfConditionsRequired && needOneOfConditionsAchieved);
			}
			if (patternDetected) {
	...
	private void getFilesAndDirectoriesFromRoot(WorkContext context) {
		...
		try {
			Files.walkFileTree(Paths.get(((String) context.get(WorkFlowConstants.START_DIRECTORY.toString()))),new CollectFiles(fileList, directoryList));
		} catch (IOException e) {
			log.error("Unable to get the folders and files to process. Start Directory: {}",(String) context.get(WorkFlowConstants.START_DIRECTORY.toString()), e);
		}
	 ...
	 * This is done by comparing the One Of Required Clues in the Pattern and the DETECTED_CLUES
	 * in the WorkContext
	 */
	...
	/*
	 * Checks a Pattern to see if the 'All Required Clues' have been detected.
	 * This is done by comparing the All Required Clues in the Pattern and the DETECTED_CLUES
	 * in the WorkContext
	 */
