...
public class OcpOnboardingWorkFlowConfiguration {
	...
	@Bean
	OnboardingOcpAssessmentTask onboardingOcpAssessmentTask(
	...
	@Bean(name = "onboardingOcpAssessment" + WorkFlowConstants.ASSESSMENT_WORKFLOW)
	@Assessment
	WorkFlow assessmentWorkFlow(
			@Qualifier("onboardingOcpAssessmentTask") OnboardingOcpAssessmentTask onboardingOcpAssessmentTask) {
		// @formatter:off
        return SequentialFlow.Builder.aNewSequentialFlow()
                .named("onboardingOcpAssessment" + WorkFlowConstants.ASSESSMENT_WORKFLOW)
                .execute(onboardingOcpAssessmentTask)
                .build();
        // @formatter:on
	}
	...
	JiraTicketApprovalWorkFlowCheckerTask jiraTicketApprovalWorkFlowCheckerTask(
		...
		return new JiraTicketApprovalWorkFlowCheckerTask(jiraTicketApprovalEscalationWorkFlow,
				new Date().getTime() / 1000 + 60, url, username, password);
	}
