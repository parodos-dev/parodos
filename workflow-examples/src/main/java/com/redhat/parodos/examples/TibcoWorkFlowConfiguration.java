...
public class TibcoWorkFlowConfiguration {
	...
	@Bean
	TibcoMessageService tibcoMessageService(TibjmsConnectionFactory tibjmsConnectionFactory, @Value("${caFile:/opt/tibco/ems/10.2/samples/certs/server_root.cert.pem}") String caFile, @Value("${username:admin}") String username, @Value("${password:admin}") String password) {
	...
	@Bean
	@Infrastructure
	WorkFlow tibcoWorkFlow(TibcoWorkFlowTask tibcoTask) {
		// @formatter:off
