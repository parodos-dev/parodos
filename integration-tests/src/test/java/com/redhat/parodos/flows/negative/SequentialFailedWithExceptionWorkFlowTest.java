...
public class SequentialFailedWithExceptionWorkFlowTest {
	...
	private static final String WORKFLOW_NAME = "sequentialFailedWithExceptionWorkFlow";

	@Test
	public void runSequentialFailedWithExceptionWorkFlow() throws ApiException {
		log.info("******** Running The Sequential Failed With Exception WorkFlow ********");
		...
		log.info("Sequential Failed With Exception WorkFlow execution id: {}",
		...
		// verify the work flow status and message
		...
		assertThat(workFlowStatusResponseDTO.getMessage(), equalTo("FailedWithExceptionWorkFlowTask failure"));
		...
		assertThat(workFlowStatusResponseDTO.getWorks().get(0).getName(), equalTo("failedWithExceptionWorkFlowTask"));
		assertThat(workFlowStatusResponseDTO.getWorks().get(0).getStatus(),
				equalTo(WorkStatusResponseDTO.StatusEnum.FAILED));
		assertThat(workFlowStatusResponseDTO.getWorks().get(0).getMessage(),
				equalTo("FailedWithExceptionWorkFlowTask failure"));

		log.info("******** Sequential Failed With Exception WorkFlow successfully failed: {} ********",
	...
	private static Consumer<WorkFlowDefinitionResponseDTO> getWorkFlowDefinitionResponseConsumer() {
		return workFlowDefinition -> {
			...
			assertThat(firstWork.get().getName(), equalTo("failedWithExceptionWorkFlowTask"));
